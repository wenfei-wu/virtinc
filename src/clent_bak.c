...
void add_to_send(ip_pcb_t *ip) {
  pthread_mutex_lock(&send_buffer_mutex);
  send_buffer_tail->next = malloc(sizeof(ip_pcb_t));
  memcpy(send_buffer_tail->next, ip, sizeof(ip_pcb_t));
  send_buffer_tail = send_buffer_tail->next;
  send_buffer_tail->next = NULL;
...
void process_window(task_t *task, int seq) {
  ...
  for(int now = task->p % WINDOW_SIZE; task->ack_mask >> now & 1; now = (now + 1) % WINDOW_SIZE) {
    ...
    if(task->p + WINDOW_SIZE < task->packet_num)
      make_and_send(task, nowtask->p + WINDOW_SIZE);
    ++task->p;
  }
...
void send_tle_packet(task_t *task) {
  ...
  for(int i = 0; i < WINDOW_SIZE; ++i) 
    if(task->time[i] + MAX_DELAY < now) {
      add_to_send(task->packet[i]);
      time[i] = now;
    }
}

void pre_send(task_t *task) {
  for(int i = 0; i < WINDOW_SIZE && i < packet_num; ++i)
...
void make_and_send(task_t *task, int p) {
  ...
  if(p == task->packet_num - 1) // last packet
    size = task->size - INCP_PAYLOAD * (packet_num - 1);
  in_pcb_t *sd = task->packet[p % WINDOW_SIZE];
  ...
  fread(((ip_pcb_t *)(sd->data))->data, size, 1, task->fp);
...
void * run_sender(void * conn_id){
  ...
  while(tasks[id].state) {
	...
	if(tasks[id].state != 2) {
	  ...
	  process_window(&tasks[id], ((incp_header_t *)sd.data)->seq_num);
      }
...
void clean_exit() {
  ...
  //buffer
  ip_pcb_t* prev = send_buffer_head;
  ...
  for(int i = 0; i < conn_num; ++i)
    pthread_mutex_destory(&recv_mutex[i]);
...
void init_buffer() {
  ...
  send_buffer_tail = send_buffer_head;
  for(int i = 0; i < conn_num; ++i) {
...
void* send_buffer_packet(void *iiiiii) {
  ip_pcb_t *current;
  while(1) {
...
int main(int argc, char** argv){
  ...
  pthread_mutex_init(&send_buffer_mutex, NULL);
  ...
  res = pthread_create(&p_send_buffer, NULL, send_buffer_packet, NULL);
  if(res < 0){
    printf("pthread_create error: %d\n", res);
    clean_exit();
  }
  ...
  pthread_kill(p_send_buffer, SIGKILL);
  ...
  pthread_join(p_send_buffer, NULL);
